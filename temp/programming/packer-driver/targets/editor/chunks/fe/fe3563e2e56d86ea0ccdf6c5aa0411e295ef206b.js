  ...
  return {
    ...
    execute: function () {
      ...
      _export("shoot", shoot = (_dec = ccclass('shoot'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Label), _dec(_class = (_class2 = class shoot extends Component {
        ...
        onLoad() {
          // 將HIT字串設定為空值
          this.hit.string = ""; // 將bulletAry陣列設定為空值
        ...
        update(deltaTime) {
          for (let i = 0; i < this.bulletAry.length; i++) {
            let addbullet = this.bulletAry[i - 1];
        ...
        onclickshoot(evt) {
          ...
          let addbullet = instantiate(this.bulletPrefab); // 在場景中新增子彈節點
        ...
        collision(addbullet, enemy) {
          // 計算子彈與敵人間的距離
          let dist = addbullet.position.subtract(enemy.position).length(); // 將計算結果傳入dist
...
//# sourceMappingURL=fe3563e2e56d86ea0ccdf6c5aa0411e295ef206b.js.map
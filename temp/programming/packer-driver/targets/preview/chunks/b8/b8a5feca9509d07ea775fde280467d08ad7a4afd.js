System.register(["cc"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _export("shoot", shoot = (_dec = ccclass('shoot'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Prefab), _dec7 = property(Prefab), _dec8 = property(Label), _dec9 = property(Label), _dec10 = property(Label), _dec11 = property(Label), _dec12 = property(Prefab), _dec(_class = (_class2 = class shoot extends Component {
        constructor() {
          super(...arguments);
        ...
        update(deltaTime) {
          for (var i = this.bulletAry.length - 1; i >= 0; i--) {
            ...
            if (this.hit(addbullet, this.addenemy)) {
              // 清除陣列中檢查完的擊中事件
        ...
        startgame() {
          console.log("startgame"); // 每0.5秒將addbullet新增至場景,複製bulletPrefab(子彈)至addbullet中 

          this.schedule(() => {
            var addenemy = instantiate(this.enemy);
          ...
          this.scheduleOnce(() => {
            ...
            this.startview.active = false;
            this.endview.active = false;
            this.gameview.active = true; // 遊戲倒數
        ...
        onKeyDown(event) {
          if (event.keyCode === KeyCode.SPACE) {
            console.log("按下SPACE");
            var clickshoot = new Event('click');
        ...
        onclickshoot(event) {
          if (this.bulletcount > 0) {
            ...
            bulletAni.on(Animation.EventType.FINISHED, () => {
              ...
              var addmiss = instantiate(this.missPrefab); // 將addmiss新增至場景

              this.node.addChild(addmiss); //0.3秒後移除MISS
        ...
        collision(addbullet, addenemy) {
          // 計算子彈與敵人間的距離
          var dist = addbullet.position.subtract(addenemy.position).length(); // 回傳dist數值

          return dist;
        } // 定義擊中事件


        hit(addbullet, addenemy) {
          if (this.collision(addbullet, addenemy) < 100) {
            console.log("Hit");
...
//# sourceMappingURL=b8a5feca9509d07ea775fde280467d08ad7a4afd.js.map